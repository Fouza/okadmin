...
class Exercise extends Component {
  ...
  async getCategoriesExercises() {
    ...
    await axios.get(`http://127.0.0.1:888/api/service/categorieExercise/listCategoriesExercises`,
      config).then(
        res => {
  ...
  async addCategorieExercise() {
    ...
    await axios.post(`http://127.0.0.1:888/api/service/categorieExercise/addCaetgoryExercise`,
      {
  ...
  async removeCategorieExercise(id, index) {
    ...
    await axios.delete(`http://127.0.0.1:888/api/service/categorieExercise/deleteCategoryExercise/${id}`,
      config).then(
        res => {
          //console.log(res.data)
  ...
  async modifierCategorieExercise(id) {
    ...
    await axios.put(`http://127.0.0.1:888/api/service/categorieExercise/updateCategoryExercise/${this.state.cat_id}`,
      {
  ...
  handleSupprimer(e) {
    ...
    this.removeCategorieExercise(e.target.id, parseInt(e.target.name));
  }
  ...
  render() {
    ...
    const tabPane = (
      <>
        <TabPane tabId="1">
          <Card>
            <CardHeader>Ajouter une catégorie d'exercise</CardHeader>
            <CardBody>
              <FormGroup>
        ...
        <TabPane tabId="2">
          <Card>
            <CardHeader>Modifier une catégorie d'exercise</CardHeader>
            <CardBody>
              <FormGroup row>
                <Col md="3">
      ...
      <div className="animated fadeIn">
        ...
        <Row>
          <Col xs="12" lg="12">
            <Card>
              <CardHeader>
                <i className="fa fa-align-justify"></i> Catégories d'exercises
              </CardHeader>
              <CardBody>
                <Table responsive>
                  <thead>
                      ...
                      return (
                        <tr key={cat.id}>
                          ...
                          <td width="50" key={cat.id}>
                            <Row >
                              <Col>
                                <Button color="primary" block onClick={() => this.props.history.push(`/addcatexo/${cat.id}/${cat.name}`)}>
                                  <Col xs="6" sm="4" md="3" xl="2">
                                    <i className="cui-chevron-top  font-2xl d-block"></i>
                                  </Col>
                                </Button>
                                <Button id={cat.id}
                                  name={index} block outline
                                  color="danger" onClick={e => this.handleSupprimer(e)}
                                >
                                  <Col xs="6" sm="4" md="3" xl="2">
                                    <i className="cui-delete  font-2xl d-block"></i>
                                  </Col>
                                </Button>
                              </Col>
                            </Row>
